...
parser.add_argument('--data', default='temp',
...
parser.add_argument('--coeff', default=1e-2, type=float, help='Coefficient to KL term in BM loss')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        with torch.no_grad():
            model.fc.bias = torch.nn.parameter.Parameter(model.fc.bias + math.log(args.prior))
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            top1.update(acc1[0].cpu().item(), input.size(0))
            top5.update(acc5[0].cpu().item(), input.size(0))
