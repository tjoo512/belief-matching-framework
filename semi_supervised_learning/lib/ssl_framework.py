...
class SSLFramework(object):
    def __init__(
        ...
        if self.consistency_model == "pi_model_bm" or self.consistency_model == "vat_bm":
            masked_targets = tf.one_hot(masked_labels, 10)
            self.labeled_loss = tf.reduce_sum(
                self.belief_matching_loss(masked_targets, masked_logits)
    ...
    def belief_matching_loss(self, targets, logits):
        beta_coeff = 1
        coeff = 1e-2
        ...
        evi_loss = a_self - tf.digamma(a_zero)
    ...
    def prediction(self):
        ...
        with tf.variable_scope("prediction", reuse=tf.AUTO_REUSE) as var_scope:
            if self.consistency_model == "mean_teacher":
                ...
                output_student = output
                output_teacher = network_function(inputs)
            elif self.consistency_model == "pi_model_bm":
                # For pi-model, we want to make the output the same over two
                # passes
                output_student = output
                output_teacher = network_function(inputs)
            ...
            elif self.consistency_model == "vat_bm":
                r_vadv = vat_utils.generate_virtual_adversarial_perturbation(
                    inputs, output, network_function, self.hps
                )
